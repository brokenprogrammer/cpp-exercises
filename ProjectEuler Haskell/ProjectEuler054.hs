...
--          Pair of Queens, H9  Pair of Queens, H7
...
import Data.List (sort, reverse, group)
import Data.List.Split

data Suit = Spades | Clubs | Diamonds | Hearts
    deriving (Eq, Show, Ord)

data Rank = CardNumber Int | Jack | Queen | King | Ace
    deriving (Eq, Show, Ord)
...
readHand xs = map (readCard) $ splitOn " " xs

removeOccurance :: [Int] -> Int -> [Int]
removeOccurance [] _ = []
removeOccurance xs x = filter (\y -> y/= x) xs

cardOccurances :: [Int] -> [(Int, Int)]
cardOccurances [] = []
cardOccurances (x:xs) = (x, (length (filter (==x) xs))+1) : 
    (cardOccurances $ removeOccurance xs x)

valueOfCount :: Int -> [(Int, Int)] -> [Int]
valueOfCount x pairs = highToLow [num | (num, count) <- pairs, count == x]

valueOfCounts :: [Int] -> [(Int, Int)] -> [Int]
valueOfCounts (x:xs) pairs = valueOfCount x pairs

rankHand :: Hand -> (HandRank, [Int])
rankHand x =
    if isRoyalFlush then (RoyalFlush, [14, 13, 12, 11, 10])
    else if isStraightFlush then (StraightFlush, highToLow rankvalues)
    else if isFourOfAKind then (FourOfAKind, valueOfCounts [4, 1] occuranceList)
    else if isFullHouse then (FullHouse, valueOfCounts [3,2] occuranceList)
    else if isFlush then (Flush, highToLow rankvalues)
    else if isStraight then (Straight, highToLow rankvalues)
    else if isThreeOfAKind then (ThreeOfAKind, valueOfCounts [3, 1] occuranceList)
    else if isTwoPair then (TwoPair, valueOfCounts [2, 1] occuranceList)
    else if isOnePair then (OnePair, valueOfCounts [2, 1] occuranceList)
    else (HighCard, highToLow rankvalues)
    where
        numPairs  = length $ filter (==2) occurCounts
        occurCounts = map snd $ cardOccurances rankvalues
        occuranceList = cardOccurances rankvalues
        rankvalues = map (rankValue . fst) x
        suits = map head $ group $ sort (map snd x)
        isRoyalFlush = isFlush && isStraight && maximum rankvalues == 14
        isStraightFlush = isFlush && isStraight
        isFourOfAKind = 4 `elem` occurCounts
        isFullHouse = isThreeOfAKind && numPairs > 0
        isFlush = 1 == length suits
        isStraight =  length occurCounts == 5 && 
            (4 == maximum rankvalues - minimum rankvalues)
        isThreeOfAKind = 3 `elem` occurCounts
        isTwoPair = numPairs == 2
        isOnePair = numPairs == 1

doesPlayer1Win :: Hand -> Hand -> Bool
doesPlayer1Win hand1 hand2 =
    rankHand hand1 > rankHand hand2

answer = length [i | i <- hands, let x = take 14 i, let y = take 14 $ drop 15 i, 
    doesPlayer1Win (readHand x) (readHand y)]

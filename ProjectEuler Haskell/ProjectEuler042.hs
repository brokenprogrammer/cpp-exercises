...
answer = length 
         . filter (\x -> x == True) 
         . map isTriangleNumber 
         . map wordPosition $ englishWords

isTriangleNumber :: Int -> Bool
isTriangleNumber n = number == n
    where number = last $ takeWhile (<= n) triangleNumbers

triangleNumbers :: [Int]
triangleNumbers = [ div (n * (n + 1)) 2 | n <- [1..]]

wordPosition :: String -> Int
wordPosition = sum . (map (\c -> ord c - ord 'A' + 1))
